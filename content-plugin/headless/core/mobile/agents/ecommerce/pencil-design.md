...
1. Design in Pencil and define the mobile visual system
2. Mobile frontend agent reads the handoff and builds the application using Expo (React Native)
3. Existing CMS scripts connect erxes SaaS and seed content, products, categories, and navigation data

Do not change the main build logic in later steps. Your job is to make those steps more accurate for mobile implementation.
...
Use `ui_source_ref` as the creative brief. Generate the design direction from the written description and adapt it to a mobile-first experience.

### `pencil`

Open the provided `.pen` file from `ui_source_ref`. Use it as the primary screen and component reference. Improve structure only where needed for production readiness. Save the approved working copy to `output/<slug>/designs/design.pen`.

Path rule for `pencil`:

- open only the exact `.pen` path from `ui_source_ref`
- if preview variations are needed, derive them from that exact file or save them into `output/<slug>/designs/app-directions.pen`
- do not switch to another `.pen` project unless the user explicitly replaces `ui_source_ref`

### `figma`

Use the Figma link or exported assets in `ui_source_ref` as the visual source. Reconstruct the screens in Pencil and preserve the core visual language.
...
Audit the referenced site in `ui_source_ref`. Extract:
...
Adapt these patterns for mobile instead of directly cloning desktop UX.

If `design_strategy` is `copy-site` or `improve-site`, do not stop at the single URL. Discover and fetch relevant internal pages, product flows, authentication flows, and locale variants first, then use them as both design and content references.
...
Read the resulting `source-audit.json` before deciding:
...
Convert discovered website pages into Expo Router screens.
...
All extracted designs must be adapted for:
...
Do not reproduce desktop-only patterns such as:

- mega menus
- large hover interactions
- sidebar navigation
- multi-column desktop layouts

Convert them into mobile-native patterns instead.
...
## Ecommerce Screen Model — when `template_type` is `ecommerce`, design ALL of the following screens after the home screen direction is approved:

| Screen          | Route / File       | Key elements to design                                        |
| --------------- | ------------------ | ------------------------------------------------------------- |
| Home            | `/(tabs)/home`     | hero banners, categories, featured products, promotions       |
| Product listing | `/(tabs)/products` | product grid/list, filter bottom sheet, sort controls, search |
| Product detail  | `/product/[id]`    | image gallery, price, variant selector, add-to-cart, reviews  |
| Cart            | `/cart`            | line items, quantity controls, order summary, checkout CTA    |
| Checkout        | `/checkout`        | delivery form, payment method selector, order summary         |
| Login           | `/login`           | email/password form, social login options                     |
| Register        | `/register`        | registration form, terms checkbox                             |
| Profile         | `/(tabs)/profile`  | editable personal info, avatar, account settings              |
| Orders          | `/orders`          | order history list with status badges                         |
| Order detail    | `/orders/[id]`     | items, delivery status timeline, totals                       |
| Wishlist        | `/(tabs)/wishlist` | saved product grid with remove and add-to-cart actions        |
| Search          | `/search`          | search input, recent searches, results                        |
| Notifications   | `/notifications`   | notification list, order updates                              |
| Settings        | `/settings`        | language, theme, account preferences                          |

### Expo Requirements

- Design all ecommerce screens in the same Pencil file as the home screen (`output/<slug>/designs/design.pen`)
- Use Expo Router conventions for all routes
- Include mobile-first layouts for all screens
- Include dedicated mobile viewport designs for:
  - Product Detail
  - Cart
  - Checkout
  - Orders

- Cart Bottom Sheet design is required in addition to the full Cart screen
- Filter Bottom Sheet design is required for Product Listing
- Auth screens (Login, Register) must match the overall visual system but can use simplified layouts
- All screens must support Safe Area on iOS and Android
- Include loading, empty, error, and offline states for key ecommerce flows
